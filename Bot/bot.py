...
async def country(ctx, args, complete='false'):
    found = False
    response = await api_client.all_countries()
    y = sorted(response, key=lambda x: x.name.lower())
    country_name = None
    embedVar = discord.Embed(description="Statistics from disease.py",
                                color=0xe33b3b, url='https://anondoser.xyz')

    for i in y:
        if (i.name.lower() >= args.lower() and i.name.lower()[0] == args.lower()[0]):
            ...
            if complete == 'complete':
                ...
                embedVar.add_field(name ='Country', value= i.name, inline=True)
            ...
            embedVar.add_field(name='Cases:', value=check_na(
                i.cases)+cases_diff, inline=True)
            embedVar.add_field(name='Cases Today:', value=check_na(
                i.today.cases), inline=True)
...
async def all(ctx):
    ...
    embedVar.add_field(name='Cases:', value=check_na(
        response.cases)+cases_diff, inline=True)
    embedVar.add_field(name='Cases Today:', value=check_na(
        response.today.cases), inline=True)
    embedVar.add_field(name='Deaths:', value=check_na(
        response.deaths)+death_diff, inline=True)
    embedVar.add_field(name='Deaths Today:', value=check_na(
        response.today.deaths), inline=True)
    embedVar.add_field(name='Recovered:', value=check_na(
        response.recoveries)+re_diff, inline=True)
    embedVar.add_field(name='Recovered Today:', value=check_na(
        response.today.recoveries), inline=True)
    embedVar.add_field(name='Active:', value=check_na(
        response.active)+active_diff, inline=True)
    embedVar.add_field(name='Critical:', value=check_na(
        response.critical)+crit_diff, inline=True)
    embedVar.add_field(
        name='Tests:', value=check_na(response.tests)+test_diff, inline=True)
